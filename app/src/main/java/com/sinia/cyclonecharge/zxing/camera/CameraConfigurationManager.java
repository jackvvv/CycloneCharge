...
package com.sinia.cyclonecharge.zxing.camera;
...
final class CameraConfigurationManager {
	...
	void initFromCameraParameters(Camera camera) {
		...
		screenResolutionForCamera.x = theScreenResolution.x;
		screenResolutionForCamera.y = theScreenResolution.y;
		if(theScreenResolution.x < theScreenResolution.y){
			screenResolutionForCamera.x = theScreenResolution.y;
			screenResolutionForCamera.y = theScreenResolution.x;
		}
	...
	void setDesiredCameraParameters(Camera camera, boolean safeMode) {
		...
		if (true) {
			if (safeMode || false) {
				...
				focusMode = findSettableValue(parameters.getSupportedFocusModes(), Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE, Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO,
		...
		camera.setDisplayOrientation(90);//Jacky 2014-01-07
